...
extern I2C_HandleTypeDef hi2c4;//开启I2C3的接口，你用的哪个就改哪个
...
static uint8_t    SHT30_Send_Cmd(SHT30_CMD cmd)
{
    ...
    cmd_buffer[0] = cmd >> 8;
    cmd_buffer[1] = cmd;
    return HAL_I2C_Master_Transmit(&hi2c4, SHT30_ADDR_WRITE, (uint8_t*)cmd_buffer, 2, 0xFFFF);
}
...
uint8_t SHT30_Read_Dat(uint8_t* dat)
{
    ...
    return HAL_I2C_Master_Receive(&hi2c4, SHT30_ADDR_READ, dat, 6, 0xFFFF);
}
/*************************************************
...
//     }
    ...
    {
        // 读取原始数据
        if (SHT30_Read_Dat(sht30_data) == HAL_OK)
        {
            // 解算数据
            if (SHT30_Dat_To_Float(sht30_data, &g_temperature, &g_humidity) == 0)
            {

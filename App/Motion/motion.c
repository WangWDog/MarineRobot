...
#include "cmsis_os.h"
#include <math.h>
#define  PITCH_SENSI (0.03)
#define  YAW_SENSI 5

#define  FAST_SENSI_YAW 1.2
#define  FAST_SENSI_PITCH 1.2
#define  FAST_SENSI_X 1.2
#define  FAST_SENSI_Y 1.2
#define  FAST_SENSI_DEEP 1.2

#define  SLOW_SENSI_YAW 0.5
#define  SLOW_SENSI_PITCH 0.5
#define  SLOW_SENSI_X 0.5
#define  SLOW_SENSI_Y 0.5
#define  SLOW_SENSI_DEEP 0.5
...
static const float yaw_param[3] = {0.01f, 0.0f, 0.01f};
...
void motion_task(void const* argument)
{
    motor_pwm_init();
    osDelay(100);
    PID_init(&pid_yaw, PID_POSITION, yaw_param, 0.5f, 0.2f);  //不知道要不要用 最好机械能调整明白
    ...
    while (1)
    {
        ...
        {
            roll_out = PID_calc(&pid_roll, roll, target_roll);  //如果不用rollPID 就把这行注释了

...
void dive(void) {
    // z_thrust = 0.3f;  // 正值向下推
    motor_driver(Middle_Left, 30);
    motor_driver(Middle_Right, 30);
}

void rise(void) {
    // z_thrust = -0.3f; // 负值向上推
    motor_driver(Middle_Left, 225);
    motor_driver(Middle_Right, 225);
}
...
void compute_motor_output(MotorPWMCommand_t *cmd, float x, float y,
    ...
    motor_output[Front_Right - 1] = -roll_k * roll_out + pitch_k * pitch_out - yaw_k * yaw_out + x * move_k + y *
                                    move_k;
...
void motion_task(void const *argument) {
    ...
    while (1) {
        ...
        // motor_driver(100,20);
        // char a = 'd';
        // HAL_UART_Transmit(&huart4,  (uint8_t*)&a, sizeof(a),HAL_MAX_DELAY);
        osDelay(1000); // 100Hz 控制周期
    }
}
